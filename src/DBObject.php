...
class DBObject
{
    ...
    public function create()
    {
        ...
        foreach ($row as $key => $value) {
            ...
            if (property_exists($this, $name)) {
                //error_log(sprintf("TEST ROW: %s", $this->$name));
        ...
        //error_log(sprintf("InsertSQL res: %s\n", $inserted));
    ...
    public function checkMeIn($table)
    {
        ...
        $row = $this->dbHandler->getRowSql($sql);
        //error_log(sprintf("ROW: %s vs SQL %s, myID: [%d]<br>\n", json_encode($row), $sql, $this->id));
    ...
    public function getChildren($type = null)
    {
        foreach ($this->childrenClassArray as $key => $value) {
           ...
           // error_log(sprintf("CHILD CLASS: %s", $tableName));
